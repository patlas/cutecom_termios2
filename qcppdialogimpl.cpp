...
void QCPPDialogImpl::connectTTY()
{
   ...
   if (m_applyCb->isChecked())
   {
      ...
      int ret = ioctl(m_fd, TCGETS2, &m_oldtio);
      if (ret != 0)
      {
         std::cerr<<"Read termio2 setting struct error"<<std::endl;
      }
...
//   memset(&newtio, 0, sizeof(newtio));
   ...
   int ret = ioctl(m_fd, TCGETS2, &newtio);

   if (ret != 0)
   {
      std::cerr<<"Read (2) termio2 setting struct error"<<std::endl;
   }
...
//      _baud=B256000;
   ...
   int r = ioctl(m_fd, TCSETS2, &newtio);

   if(r != 0)
   {
       std::cerr<<"Set (1) termio2 setting struct error"<<std::endl;
   }
...
//   }
   ...
   r = ioctl(m_fd, TCSETS2, &newtio);

   if(r != 0)
   {
       std::cerr<<"Set (2) termio2 setting struct error"<<std::endl;
   }
...
//   }
   ...
   if(r != 0)
   {
       std::cerr<<"Set (3) termio2 setting struct error"<<std::endl;
   }
